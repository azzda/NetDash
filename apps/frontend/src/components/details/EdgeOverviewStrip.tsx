    ...
    <article className="surface-subtle rounded-xl p-3">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <h4 className="mt-1 text-base font-semibold text-primary">{endpoint?.label ?? "Unmapped endpoint"}</h4>
        </div>
      ...
      <dl className="grid gap-3 text-sm sm:grid-cols-2">
        <div>
          ...
          <dd className="mt-1 font-medium text-primary">{ipSummary}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-[0.16em] text-dimmed">DNS</dt>
          <dd className="mt-1 font-medium text-primary">{dnsSummary}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-[0.16em] text-dimmed">Physical Port</dt>
          <dd className="mt-1 font-medium text-primary">{endpoint?.physicalPort ?? "Not specified"}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-[0.16em] text-dimmed">Logical Port</dt>
          <dd className="mt-1 font-medium text-primary">{endpoint?.logicalPort ?? "Not specified"}</dd>
        </div>
      </dl>
    </article>
  );
...
export function EdgeOverviewStrip({ edge, densityPreference, onClose }: EdgeOverviewStripProps) {
  ...
  return (
    <section className={`surface-card rounded-xl ${paddingClass}`}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-dimmed">Connector Overview</p>
          <h3 className="mt-1 text-lg font-semibold text-primary">{data?.displayName ?? "Connection"}</h3>
          <p className="mt-1 text-xs text-dimmed">UUID {data?.connectorUuid ?? edge.id}</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1fr)_260px_minmax(0,1fr)]">
        <EndpointSummaryCard title="Side A" endpoint={data?.sideA} />

        <article className="surface-subtle connector-flow-card rounded-xl p-3">
      ...
      <section className="mt-4 rounded-xl border border-white/10 bg-white/5 px-3 py-3">
