...
export function UsagePriceDrawer({
  ...
  const avgPower = useMemo(
    () =>
      usageSeries.reduce((sum, point) => sum + point.powerWatts, 0) /
  ...
  return (
    <div className="fixed inset-0 z-[34] flex justify-end bg-black/30" onClick={onClose}>
      <aside
        className="settings-drawer h-full w-full max-w-md overflow-y-auto border-l px-4 py-4 sm:px-5"
        ...
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-dimmed">
              Power and cost placeholders for {userProfile.displayName}.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <article className="surface-subtle rounded-xl p-3">
              ...
              <p className="mt-1 text-xl font-semibold text-primary">
                {formatCurrency(totalCost, currencyPreference)}
              </p>
            </article>
            <article className="surface-subtle rounded-xl p-3">
          ...
          <svg viewBox={`0 0 ${width} ${height}`} className="h-32 w-full overflow-visible">
            ...
            {points.map((point) => (
              <circle
                ...
                cy={point.y}
                r="3.6"
                fill="var(--edge-activity-secondary)"
              >
          ...
          <section className="surface-card mt-4 space-y-3 p-3">
            <div>
              ...
              <p className="mt-1 text-xs text-dimmed">
                Simple placeholder calculations for service allocation and trend tracking.
              </p>
            </div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="px-3 py-2 text-primary">
                      {formatCurrency(totalCost * 0.42, currencyPreference)}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="px-3 py-2 text-primary">
                      {formatCurrency(totalCost * 0.38, currencyPreference)}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="px-3 py-2 text-primary">
                      {formatCurrency(totalCost * 0.2, currencyPreference)}
                    </td>
                  </tr>
                </tbody>
              </table>
